import { type Ref, ref, type ComputedRef, ComputedGetter, computed } from 'vue'
import {
...
// TODO fix nested arrays, somewhere around the simple K is the error
type ArrayFieldNames<T> = {
  [K in keyof T]: T[K] extends ArrayFormField<any, infer R, any, any>
    ? R extends FormFieldBase<any, any, any>
      ? K
      : R extends FormFieldSetRaw
      ? K | `${K & string}.0.${ArrayFieldNames<R> & string}`
      : never
    : T[K] extends FormFieldSetRaw
...
type DataTypeFromFormFieldSetRaw<T extends readonly unknown[], K extends FormFieldSetRaw> = T['length'] extends 1
  ? T[0] extends keyof K
    ? GetReturnTypeIfFunction<K[T[0]]['getDefault']>[number]
    : never
  : T extends [infer R, ...infer P]
  ? R extends keyof K & string
    ? K[R] extends FormFieldSetRaw
      ? DataTypeFromFormFieldSetRaw<P, K[R]>
      : K[R] extends ArrayFormField<any, any, any, any>
      ? K[R]['baseField'] extends FormFieldSetRaw
...
type PushRelatedArguments<T extends string, FS extends FormFieldSetRaw> = [
...
export class FormDefinition<FS extends FormFieldSetRaw, EMD extends FormExtraMethodsDefinition<Form<FS>>> {
  readonly fieldSet: FormFieldSet<FS>
  readonly extraMethods: EMD

  constructor(rawFieldSet: FS, extraMethods?: EMD) {
    this.fieldSet = new FormFieldSet(rawFieldSet)
...
export class Form<FS extends FormFieldSetRaw> {
  ...
  #unflattenFieldSetErrors(fieldSet: FormFieldSet<any, any>, accessors: string[]): Record<string, any> {
    const errors: Record<string, any> = {}

    for (const [fieldName, field] of Object.entries(fieldSet.fieldSetRoot)) {
      if (field instanceof FormFieldSet) {
